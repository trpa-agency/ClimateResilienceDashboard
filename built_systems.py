...
def get_data_mode_share():
    ...
    modeshare_data_grouped = modeshare_data.groupby(['Year','Season', 'Day', 'Mode', 'Source']).agg({'Number': 'mean'}).reset_index()
    modeshare_data_grouped['Total'] = modeshare_data_grouped.groupby(['Year','Season','Day', 'Source'])['Number'].transform('sum')
...
def plot_mode_share(df):
    ...
    fig.add_trace(go.Bar(
        ...
        name='LOCUS',
        showlegend=False,
        customdata = np.stack((modeshare_data_locus['Mode'],modeshare_data_locus['Percentage']/100), axis=-1),
    ...
    fig.add_trace(go.Bar(
        ...
        name='Replica',
        showlegend=False,
        customdata = np.stack((modeshare_data_replica['Mode'],modeshare_data_replica['Percentage']/100), axis=-1),
        hovertemplate=hovertemplate_text,
        visible=False,
        marker=dict(
    ...
    fig.add_trace(go.Bar(
        ...
        name='Survey',
        showlegend=False,
        customdata = np.stack((modeshare_data_survey['Mode'],modeshare_data_survey['Percentage']/100), axis=-1),
        hovertemplate=hovertemplate_text,
        visible=False,
        marker=dict(
            ...
            dict(active=0,
                buttons=list([
                    ...
                    dict(label="Source: Replica",
                        method="update",
                        args=[{"visible": [False, True,False,True, True,True, True,True]},{
                                'shapes[0].visible': False, 'shapes[1].visible': True,
                            'shapes[2].visible': False}]),
                    dict(label="Source: TRPA Survey",
                        method="update",
